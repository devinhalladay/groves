...
const ChannelIndex = (props) => {
  ...
  const handleChannelCheckbox = (e, channel) => {
    if (
      selections.length &&
      selections.findIndex((c) => c.id === channel.id) !== -1
    ) {
  ...
  const handleDeletion = (channel) => {
    selections.length > 1
      ? handleDeleteMultipleChannels()
      : handleDeleteSingleChannel(channel);
  };

  const handleMultiSelect = (channel, i) => {
    const start = flatIndex.findIndex(
      (c) => c.id === selections[selections.length - 1].id,
    );
    const end = i;
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <Button
                        icon="trash"
                        onClick={() => handleDeletion(channel)}
                      />
                    </div>
                  </Card>
                </Checkbox>
