...
export default class ArenaClient {
  ...
  getChannelsForMe() {
    ...
    return new Promise(async (resolve, reject) => {
      await this._makeRequest(
        "get",
        `/users/${this.me.id}/channels?per=${amountPerPage}&page=${initialPage}`
      ).then(async (resp) => {
        if (resp.data.total_pages === 1) {
          ...
          for (let i = initialPage + 1; i <= resp.data.total_pages; i++) {
            await this._makeRequest(
              "get",
              `/users/${this.me.id}/channels?per=${amountPerPage}&page=${i}`
            ).then((resp) => {
              channelsArray.push(...resp.data.channels);
            });
          }
  ...
  async getBlocksFromChannel(channelID, channelLength) {
    ...
    return new Promise(async (resolve, reject) => {
      await this._makeRequest(
        "get",
        `/channels/${channelID}/contents?per=${amountPerPage}&page=${initialPage}`
      ).then(async (resp) => {
        ...
        if (pageCount === 1) {
          ...
          for (let i = initialPage + 1; i <= pageCount; i++) {
            await this._makeRequest(
              "get",
              `/channels/${channelID}/contents?per=${amountPerPage}&page=${i}`
            ).then((resp) => {
              blocksArray.push(...resp.data.contents);
            });
          }
